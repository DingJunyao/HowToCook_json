---
name: parse-single-recipe
description: 解析 HowToCook 项目中的单个 Markdown 菜谱文件，提取菜谱信息并生成符合 scripts/recipe_schema.json 格式的 JSON 数据。
---

# parse-single-recipe 技能

## 目标
解析 HowToCook 项目中的单个 Markdown 菜谱文件，提取菜谱信息并生成符合 scripts/recipe_schema.json 格式的 JSON 数据，写入到给定文件中。
...
## 输出
- 在给定文件路径保存符合菜谱 Schema 的 JSON 数据，严格遵循 scripts/recipe_schema.json 这个 Schema。包含以下字段：
  - name: 菜谱名称
...
## 重要约束
- 由于 markdown 文件里面的格式不统一，所以请**直接**分析，**禁止**书写脚本分析。
- 每道菜谱至少有一个原料和一个步骤
...
- 菜谱解析必须输出 JSON 格式的内容，且 JSON 严格遵循 scripts/recipe_schema.json 这个 Schema
- 如果菜谱附有图片，也需提取图片链接
- 如果菜谱没有提到分量，默认为一人份